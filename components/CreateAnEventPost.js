...
export default function CreateAnEventPost(props) {
      ...
      uploadImagesAndGetUrls(images)
        .then(async (urls) => {
          let postData = {
            ...
            content: content,
            date: date,
            type: "event", //photo, file, text, poll, ?event?
  ...
  const handleConfirmDatePicker = (event, date) => {
    ...
    setDateToShow(new Date(getBirthday.toString()));
  ...
  return (
    ...
    >
      <CustomTextInput
        placeholder={`create a ${visibility} event...`}
        value={text}
        multiline={true}
        numberOfLines={4}
        editable={!loading}
      ...
      {isDatePickerVisible && (
        <DateTimePicker
          value={dateToShow ? dateToShow : new Date()}
